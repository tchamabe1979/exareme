...
drop table if exists defaultDB.datasets;
create table defaultDB.datasets as
...
from %{input_local_tbl}  where colname in ('%{variable}','dataset');
...
drop table if exists defaultDB.tempinputlocaltbl;
create table defaultDB.tempinputlocaltbl as
select * from tempinputlocaltbl1
         where rid in (select rid from tempinputlocaltbl1 where colname = 'dataset' and val in (select d from defaultDB.datasets));

drop table if exists defaultDB.lala ;
create table defaultDB.lala as select * from defaultDB.tempinputlocaltbl;

var 'totalcount' from select count(rid) from defaultDB.tempinputlocaltbl;

drop table if exists  inputlocaltbl;
create table  inputlocaltbl as
select * from defaultDB.tempinputlocaltbl
where val <> 'NA'     
 	and val is not null      
	and val <> ""
	and val not in (select d from defaultDB.datasets)
order by rid, colname,val;


drop table if exists defaultDB.sofia;
create table defaultDB.sofia as
select * from inputlocaltbl;
