...
#    EXAREME_MASTER=`/sbin/ifconfig $1 | grep "inet " | awk -F: '{print $2}'  | grep '83.212.171.200' | awk '{print $1;}' | head -n 1`;	#TODO 10.20 always?
#fi
#todo think if always there is an IP in ~/exareme-standalone/runExaremenside pwd/etc/exareme/master
EXAREME_MASTER=$( < $EXAREME_MASTER_FILE)
echo "EXAREME_MASTER" $EXAREME_MASTER
...
if [[ "true" == $EXAREME_ADMIN_LOCAL ]]; then   # run locally
   # echo "Running in local mode..."
    ...
    function start_exareme(){ # starts exareme daemon
        # set env
        EXAREME_CURRENT_IP=$(		
            ALL_IPS=$(wget http://ipinfo.io/ip -qO -)
        ...
        echo $EXAREME_CURRENT_IP
        #if [ -n $EXAREME_CURRENT_IP ]; then
        #    EXAREME_CURRENT_IP=`/sbin/ifconfig $1 | grep "inet " | awk -F: '{print $2}'  | grep '83.212.171.200' | awk '{print $1;}' | head -n 1`;
        #fi
